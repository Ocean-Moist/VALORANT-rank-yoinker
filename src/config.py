            ...
            with open("config.json", "r") as file:
                ...
                if len(default_keys) != len(keys):
                    ...
                    with open("config.json", 'w') as w:
                        missingkeys = list(filter(lambda x: x not in keys,
                                                  default_keys))  # comparing the keys in the file to the keys in the default and returning the missing keys
                        self.log("missing keys: " + str(missingkeys))
                ...
                if config.get("weapon").strip() == "":
                    weapon = input(
                        "Enter the name of the weapon you use the most (This is for tracking the skins): ").capitalize().strip()
